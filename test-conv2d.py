...
w_tf = np.random.randn(1, filter_shape[0], filter_shape[0], depth, filters).astype(np.float32)
...
# ============
# Create conv2d layer
layer_tf = tf.keras.layers.Conv2D(filters=filters, kernel_size=filter_shape, padding='same', strides=(1, 1), use_bias=False)
